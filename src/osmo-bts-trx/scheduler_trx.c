	       ...
	       uint8_t bid, const struct trx_ul_burst_ind *bi)
{
	...
	*mask = 0x0;
	...
	case GSM48_CMODE_SPEECH_AMR: /* AMR */
		 ...
		 * since it only contains a recognition pattern. Instead, the
		 * voce frame after the AFS_ONSET frame actually is the first
		 * real voice frame that is will also result into an RTP frame. */
